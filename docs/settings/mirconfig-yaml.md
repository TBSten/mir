...
```yaml
# yaml-language-server: $schema=https://raw.githubusercontent.com/TBSten/mir/refs/heads/main/schema/v1/mirconfig.schema.json
```
...
| `defaults.<変数名>` | `string` | No | - | 任意のテンプレート変数のデフォルト値 |
...
# 任意のテンプレート変数名をキーとして設定可能
defaults:
  author: string          # デフォルト author
  <変数名>: string        # 任意のテンプレート変数のデフォルト値
```
...
snippet 作成・インストール時に使用するデフォルト値。`author` の他に、任意のテンプレート変数名をキーとして設定できる。

設定した値は `mir install` 時にテンプレート変数のデフォルト値として使用される。優先順位は以下の通り:

1. CLI 引数 (`--key=value`) — 最優先
2. `defaults` の値
3. snippet 定義の `schema.default`
...
任意のテンプレート変数のデフォルト値。例えば `packageName` を設定すると、その変数を持つ snippet のインストール時にデフォルト値として使用される。
